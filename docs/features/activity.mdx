...
Activity has two tabs: **Tasks** (the kanban above) and **Usage**. The switcher sits just below the page header and preserves the current agent + window filters when you move between tabs.

Usage is the same shape as Tasks — tabs → summary cards → filter → detail — but instead of kanban columns it shows token consumption rolled up for the selected window:
