...
Structured facts the agent learns from conversations - it writes some down deliberately, and reflects on finished threads to save others on its own (including lessons from 👍 / 👎 feedback). Relevant memories are injected into the prompt every turn, filtered to whoever is asking, so the agent always applies what it has learned - no skill call, no retrieval step.
...
2. **Memories** load every turn, injected into the prompt and filtered to the current sender. The agent always applies what it has learned.
