...
type Tab = 'general' | 'users' | 'env-vars';
  ...
  return (
    <div className="fade-up" style={{ padding: '36px 40px' }}>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--border)', marginBottom: 24 }}>
        ...
        {canEdit && <TabBtn active={tab === 'env-vars'} onClick={() => setTab('env-vars')}>Env Vars</TabBtn>}
        {canManageUsers && <TabBtn active={tab === 'users'} onClick={() => setTab('users')}>Users</TabBtn>}
      </div>

      {tab === 'general' && <GeneralTab />}
      {tab === 'env-vars' && canEdit && <EnvVarsTab />}
...
function EnvVarsTab() {
  ...
  const remove = async (key: string) => {
    if (!confirm(`Delete env var "${key}"? MCPs referencing it will stop receiving this value.`)) return;
    ...
    <>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
        <p style={{ margin: 0, fontSize: 13, color: 'var(--muted)', maxWidth: 520 }}>
          Named secrets shared across all MCP servers. Values are write-only — never shown after saving.
          Reference them in MCP stdio env vars using <code style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>ref</code> mode.
        </p>
        {!showForm && (
          <button onClick={() => setShowForm(true)} style={{
            background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8,
            padding: '8px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer',
            fontFamily: 'var(--font-sans)', flexShrink: 0, marginLeft: 16,
          }}>+ Add Env Var</button>
        )}
      </div>

      {showForm && (
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: 20, marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{editKey ? `Edit ${editKey}` : 'New Env Var'}</span>
            <button onClick={resetForm} style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: 18, cursor: 'pointer' }}>×</button>
          </div>
          {error && <div style={{ fontSize: 12, color: '#dc2626', background: 'rgba(220,38,38,0.06)', padding: '6px 10px', borderRadius: 6, marginBottom: 12 }}>{error}</div>}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--muted)', marginBottom: 5 }}>Key *</label>
              <input
                value={form.key} onChange={e => setForm(f => ({ ...f, key: e.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, '_') }))}
                placeholder="DATABASE_URL" readOnly={!!editKey}
            ...
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--muted)', marginBottom: 5 }}>
                Value {editKey ? '(leave blank to keep existing)' : '*'}
              </label>
              <input type="password" value={form.value} onChange={e => setForm(f => ({ ...f, value: e.target.value }))}
                placeholder={editKey ? '••••••••' : 'Enter secret value'} style={{ ...inputSt, fontFamily: 'var(--font-mono)' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--muted)', marginBottom: 5 }}>Description</label>
              <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                placeholder="What is this secret for?" style={inputSt} />
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            ...
            <button onClick={resetForm} style={{
              ...
              borderRadius: 7, padding: '8px 20px', fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font-sans)',
            }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ border: '1px dashed var(--border)', borderRadius: 10, padding: 32, textAlign: 'center', color: 'var(--subtle)' }}>
          <p style={{ margin: '0 0 4px', fontSize: 14, fontWeight: 500, color: 'var(--muted)' }}>No env vars yet</p>
          <p style={{ margin: 0, fontSize: 13 }}>Add secrets here and reference them in MCP configs instead of pasting raw values.</p>
        </div>
      ) : (
        <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
          {vars.map((v, i) => (
            <div key={v.key} style={{
              ...
              <code style={{ fontSize: 12.5, fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--text)', flex: '0 0 auto' }}>{v.key}</code>
              ...
              <button onClick={() => startEdit(v)} style={{ background: 'none', border: 'none', color: 'var(--accent)', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>Edit</button>
              <button onClick={() => remove(v.key)} style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>Delete</button>
            </div>
          ))}
        </div>
      )}
