...
interface SensitiveFlow {
  ...
  sourceLabel: string; sinkLabel: string; sinkSpanId: string;
  sessionId: string | null; agentName: string | null; sessionSummary: string | null; startMs: number;
}
...
function Body(): React.JSX.Element {
  ...
  const [flows, setFlows] = useState<SensitiveFlow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    if (r.ok) { const body = await r.json(); setEvents(body.events ?? []); setFlows(body.flows ?? []); }
    setLoaded(true);
    ...
    <div className="fade-up" style={{ padding: '36px 40px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {flows.length > 0 && (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 8px', display: 'flex', alignItems: 'center', gap: 6 }}>
            <ArrowRight size={13} /> Exfiltration flows
            <span style={{ fontWeight: 500, textTransform: 'none', letterSpacing: 0, color: 'var(--subtle)' }}>— sensitive data that travelled source → sink</span>
          </div>
              ...
              <Link key={f.id} href={`/activity/${encodeURIComponent(f.sessionId ?? '')}?span=${encodeURIComponent(f.sinkSpanId)}`}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', fontSize: 12.5 }}>
                    ...
                    <code style={{ fontSize: 11.5, color: 'var(--muted)', fontFamily: 'var(--font-mono, monospace)' }}>{f.sourceLabel}</code>
                    <ArrowRight size={12} style={{ color: 'var(--red)' }} />
                    <code style={{ fontSize: 11.5, color: 'var(--muted)', fontFamily: 'var(--font-mono, monospace)' }}>{f.sinkLabel}</code>
                  </div>
                  <div style={{ marginTop: 3, fontSize: 12, color: 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {f.agentName ?? 'agent'}{f.sessionSummary ? <span style={{ color: 'var(--subtle)' }}> · {f.sessionSummary}</span> : null}
                  </div>
                </div>
                <span style={{ fontSize: 11, color: 'var(--subtle)', flexShrink: 0 }}>{relativeTime(f.startMs)}</span>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        ...
        {[...events].sort((a, b) => (SEV_RANK[b.severity ?? 'low'] ?? 0) - (SEV_RANK[a.severity ?? 'low'] ?? 0) || b.startMs - a.startMs).map((e, i) => {
