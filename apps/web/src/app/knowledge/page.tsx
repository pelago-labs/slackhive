...
export default function KnowledgePage() {
  ...
  async function saveSource() {
    ...
    if (sourceForm.type === 'repo') {
      body.repoUrl = sourceForm.repoUrl; body.branch = sourceForm.branch;
      if (sourceForm.patEnvRef) body.patEnvRef = sourceForm.patEnvRef;
    }

    if (editingSource) {
      const r = await fetch(`/api/wiki-folders/${selected.id}/sources/${editingSource.id}`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
      });
      const updated = await r.json();
      setSources(prev => prev.map(s => s.id === updated.id ? updated : s));
    } else {
      const r = await fetch(`/api/wiki-folders/${selected.id}/sources`, {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', minHeight: 0 }} className="fade-up">
      ...
      {/* ── Folder list ───────────────────────────────────────────────────── */}
      <div style={{ width: 260, flexShrink: 0, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--surface-2)' }}>
        <div style={{ padding: '18px 14px 12px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {canEdit && (
            <button onClick={() => { setFolderForm({ name: '', description: '' }); setShowNewFolder(true); }} style={{ background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 9px', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>+ New</button>
          )}
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '6px' }}>
          {loading ? (
            ...
            <div key={f.id} onClick={() => selectFolder(f)} style={{ padding: '9px 10px', borderRadius: 7, cursor: 'pointer', marginBottom: 1, background: selected?.id === f.id ? 'var(--surface-3)' : 'transparent', border: selected?.id === f.id ? '1px solid var(--border)' : '1px solid transparent' }}>
            ...
            <div style={{ padding: '20px 28px 0', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                <div>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--subtle)', marginTop: 4 }}>Owner: <span style={{ color: 'var(--muted)', fontWeight: 500 }}>{selected.createdBy}</span></div>
                </div>
                {canManageSelected && (
              ...
              <div style={{ display: 'flex' }}>
                {(['sources', 'wiki'] as const).map(tab => (
                  <button key={tab} onClick={() => switchTab(tab)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '7px 16px', fontSize: 13, fontWeight: detailTab === tab ? 600 : 400, color: detailTab === tab ? 'var(--text)' : 'var(--muted)', borderBottom: `2px solid ${detailTab === tab ? 'var(--accent)' : 'transparent'}`, marginBottom: -1 }}>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '20px 28px' }}>
              {detailTab === 'sources' && (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    ...
                    {canManageSelected && <button onClick={openAddSource} style={{ background: 'transparent', color: 'var(--accent)', border: '1px solid var(--accent)', borderRadius: 6, padding: '4px 10px', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>+ Add Source</button>}
                  </div>
                  {sourcesLoading ? (
                    <div style={{ color: 'var(--muted)', fontSize: 13 }}>Loading…</div>
                  ) : sources.length === 0 ? (
                    <div style={{ border: '1px dashed var(--border)', borderRadius: 10, padding: '32px', textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>
                      {canManageSelected ? 'No sources yet. Add a URL, file, or Git repo, then click Build Wiki.' : 'No sources added yet.'}
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {sources.map(s => (
                        <div key={s.id} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '13px 15px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                          <div style={{ minWidth: 0, flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                              <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', background: 'var(--surface-3)', color: 'var(--muted)', padding: '2px 6px', borderRadius: 4 }}>{s.type}</span>
                          ...
                          {canManageSelected && (
                            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                              <button onClick={() => openEditSource(s)} style={{ background: 'var(--surface-3)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', fontSize: 12, cursor: 'pointer', color: 'var(--text)' }}>Edit</button>
                              <button onClick={() => deleteSource(s.id)} style={{ background: 'transparent', border: 'none', color: 'var(--muted)', fontSize: 17, cursor: 'pointer', padding: '0 2px', lineHeight: 1 }}>×</button>
                            </div>
                          )}
                        </div>
                      ))}
              ...
              {detailTab === 'wiki' && (
                ...
                : wikiPages.length === 0 ? (
                  ...
                  <div>
                    <button onClick={() => setWikiArticle(null)} style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontSize: 13, padding: '0 0 14px' }}>← All pages</button>
                    <pre style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '20px', fontSize: 12.5, lineHeight: 1.7, overflowX: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: 'var(--text)', fontFamily: 'var(--font-mono)' }}>{wikiArticle.content}</pre>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', color: 'var(--subtle)', textTransform: 'uppercase', marginBottom: 8 }}>{wikiPages.length} page{wikiPages.length !== 1 ? 's' : ''}</div>
                    {wikiPages.map(p => (
                      <button key={p.path} onClick={() => loadWikiArticle(p)} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 9, padding: '11px 14px', textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'center' }}>
                <button
                  ...
                  style={{ background: 'var(--surface-3)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 12px', fontSize: 12, cursor: 'pointer', color: 'var(--text)', flexShrink: 0 }}
                >
                  {fileUploading ? 'Reading…' : '📎 Upload file'}
                </button>
                <span style={{ fontSize: 11.5, color: 'var(--muted)' }}>
                  {sourceForm.content ? `${sourceForm.content.length.toLocaleString()} chars loaded` : 'or paste below'}
                </span>
              </div>
              ...
              <textarea
                ...
                onChange={e => setSourceForm(p => ({ ...p, content: e.target.value }))}
          ...
          {sourceForm.type === 'repo' && (
            <>
              ...
              <select style={inputStyle} value={sourceForm.patEnvRef} onChange={e => setSourceForm(p => ({ ...p, patEnvRef: e.target.value }))}>
                ...
                {envVarKeys.length === 0 && <option disabled>No accessible env vars — add in Env Vars settings</option>}
              </select>
            </>
          )}
      ...
      {/* hidden file input lives outside the modal so it persists */}
    </div>
  );
}
...
function ModalFooter({ onCancel, onSave, saving, saveLabel, disabled }: { onCancel: () => void; onSave: () => void; saving: boolean; saveLabel: string; disabled?: boolean }) {
  return (
    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 22 }}>
      ...
      <button onClick={onSave} disabled={saving || disabled} style={{ ...primaryBtnStyle, opacity: (saving || disabled) ? 0.5 : 1 }}>
...
const primaryBtnStyle: React.CSSProperties = { background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 7, padding: '8px 18px', fontSize: 13, fontWeight: 500, cursor: 'pointer' };
const outlineBtnStyle: React.CSSProperties = { background: 'var(--surface-2)', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 7, padding: '7px 12px', fontSize: 12.5, cursor: 'pointer' };
